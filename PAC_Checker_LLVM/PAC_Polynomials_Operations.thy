...
(* Old, recursive implementation, now replaced by loop-based implemenation below *)
...
definition apl_cond :: \<open>llist_polynomial \<times> llist_polynomial \<times> llist_polynomial \<Rightarrow> bool\<close> where
  \<open>apl_cond = (\<lambda>(r,p,q). (p\<noteq>[]) \<and> (q\<noteq>[]))\<close>

definition apl_body :: \<open>llist_polynomial \<times> llist_polynomial \<times> llist_polynomial \<Rightarrow>
  (llist_polynomial \<times> llist_polynomial \<times> llist_polynomial) nres\<close> where
  \<open>apl_body = (\<lambda>(r,p,q). doN {
        ...
        if xs = ys then doN {
          ...
          if (xs,ys) \<in> term_order_rel then
            RETURN ((xs, n) # r, p, (ys, m) # q)
          else
            RETURN ((ys, m) # r, (xs, n) # p, q)
      })\<close>

definition add_poly_l1 :: \<open>llist_polynomial \<Rightarrow> llist_polynomial \<Rightarrow>
  (llist_polynomial \<times> llist_polynomial \<times> llist_polynomial) nres\<close> where
  \<open>add_poly_l1 p\<^sub>0 q\<^sub>0 \<equiv> doN {
    (r, p, q) \<leftarrow> WHILE\<^sub>T\<^bsup>(\<lambda>(r,p,q). add_poly_l' (p\<^sub>0, q\<^sub>0) = rev r @ add_poly_l' (p,q))\<^esup>
      apl_cond apl_body ([], p\<^sub>0, q\<^sub>0);
    RETURN (r,p,q)
  }\<close>

definition apl2_body :: \<open>llist_polynomial \<times> llist_polynomial \<Rightarrow>
  (llist_polynomial \<times> llist_polynomial) nres\<close> where
  \<open>apl2_body = (\<lambda>(r',s'). doN {(x, r') \<leftarrow> mop_list_pop_hd r'; RETURN (r', x # s')})\<close>

definition add_poly_l2 :: \<open>llist_polynomial \<Rightarrow> llist_polynomial \<Rightarrow> llist_polynomial \<Rightarrow> llist_polynomial nres\<close> where
  \<open>add_poly_l2 r p q \<equiv> doN {
    let s = (if p = [] then q else p);
    (_, s) \<leftarrow> WHILE\<^sub>T\<^bsup>\<lambda>(r',s').rev r' @ s' = rev r @ s\<^esup>
      (\<lambda>(r',s'). r' \<noteq> [])
      apl2_body
      (r, s);
    RETURN s
...
definition add_poly_l :: \<open>llist_polynomial \<Rightarrow> llist_polynomial \<Rightarrow> llist_polynomial nres\<close> where
  \<open>add_poly_l p q \<equiv> doN {
    (r, q, p) \<leftarrow> add_poly_l1 (COPY p) (COPY q);
    add_poly_l2 r q p
  }\<close>

subsubsection \<open>Equality with the recursive specification\<close>

fun add_poly_pref :: \<open>llist_polynomial \<times> llist_polynomial \<Rightarrow>
    llist_polynomial \<times> llist_polynomial \<times> llist_polynomial\<close> where
  \<open>add_poly_pref (p, []) = ([], p, [])\<close> |
  \<open>add_poly_pref ([], q) = ([], [], q)\<close> |
  \<open>add_poly_pref ((xs, n) # p, (ys, m) # q) =
     (if xs = ys then
        (if n + m = 0 then add_poly_pref (p, q)
         else let (r, p', q') = add_poly_pref (p, q) in ((xs, n + m) # r, p', q'))
      else if (xs, ys) \<in> term_order_rel
        then let (r, p', q') = add_poly_pref (p, (ys, m) # q) in ((xs, n) # r, p', q')
        else let (r, p', q') = add_poly_pref ((xs, n) # p, q) in ((ys, m) # r, p', q'))\<close>

lemma add_poly_pref_add_poly_l':
  \<open>case add_poly_pref (p, q) of (s, p', q') \<Rightarrow>
     s @ (if p' = [] then q' else p') = add_poly_l' (p, q)\<close>
  apply (induction \<open>(p, q)\<close> arbitrary: p q rule: add_poly_pref.induct)
  apply (auto split: prod.splits)
  done

lemma add_poly_l1_loop:
  \<open>A = rev r @ add_poly_l' (p, q) \<Longrightarrow>
    WHILE\<^sub>T\<^bsup>(\<lambda>(r,p,q). A = rev r @ add_poly_l' (p,q))\<^esup> apl_cond apl_body (r, p, q) =
      (let (s, p', q') = add_poly_pref (p, q) in RETURN (rev s @ r, p', q'))\<close>
  apply (induction \<open>(p, q)\<close> arbitrary: p q r rule: add_poly_pref.induct)
  subgoal by (subst WHILEIT_unfold) (auto simp: apl_cond_def)
  subgoal by (subst WHILEIT_unfold) (auto simp: apl_cond_def)
  subgoal
    apply (subst WHILEIT_unfold)
    apply (auto simp: apl_cond_def apl_body_def Let_def split: prod.splits)
    done
  done

lemma add_poly_l1_alt_def:
  \<open>add_poly_l1 p q = (let (s, p', q') = add_poly_pref (p, q) in RETURN (rev s, p', q'))\<close>
  unfolding add_poly_l1_def
  by (subst add_poly_l1_loop) (auto split: prod.splits)

lemma add_poly_l2_loop:
  \<open>B = rev r @ s \<Longrightarrow>
    WHILE\<^sub>T\<^bsup>\<lambda>(r',s'). rev r' @ s' = B\<^esup> (\<lambda>(r',s'). r' \<noteq> []) apl2_body (r, s) =
      RETURN ([], rev r @ s)\<close>
  apply (induction r arbitrary: s)
  subgoal by (subst WHILEIT_unfold) auto
  subgoal by (subst WHILEIT_unfold) (auto simp: apl2_body_def)
  done

lemma add_poly_l2_alt_def:
  \<open>add_poly_l2 r p q = RETURN (rev r @ (if p = [] then q else p))\<close>
  unfolding add_poly_l2_def
  by (subst add_poly_l2_loop) auto

lemma add_poly_l_add_poly:
  \<open>add_poly_l p q = RETURN (add_poly_l' (p, q))\<close>
...
text \<open>\<open>add_poly_l_add_poly\<close> is proven right after the definition, above.\<close>

lemma add_poly_l_add_poly_uncurry:
  \<open>uncurry add_poly_l x = RETURN (add_poly_l' x)\<close>
  by (cases x) (simp add: add_poly_l_add_poly)

lemma add_poly_l_spec:
  \<open>(uncurry add_poly_l, uncurry (\<lambda>p q. SPEC(\<lambda>r. add_poly_p\<^sup>*\<^sup>* (p, q, {#}) ({#}, {#}, r)))) \<in>
    sorted_poly_rel \<times>\<^sub>r sorted_poly_rel \<rightarrow>\<^sub>f \<langle>sorted_poly_rel\<rangle>nres_rel\<close>
  apply (intro nres_relI frefI)
  apply (simp only: add_poly_l_add_poly_uncurry)
...
text \<open>Like with addition, we create an alternative version with WHILE.\<close>

definition mc_body :: \<open>llist_polynomial \<times> llist_polynomial \<Rightarrow>
  (llist_polynomial \<times> llist_polynomial) nres\<close> where
  \<open>mc_body = (\<lambda>(r, p). doN {
        ((xs, n), p) \<leftarrow> mop_list_pop_hd p;
        if p = [] then
          if n = 0 then
            RETURN (r, p)
...
definition merge_coeffs1 :: \<open>llist_polynomial \<Rightarrow> llist_polynomial nres\<close> where
  \<open>merge_coeffs1 p \<equiv> doN {
...
lemma merge_coeffs1_loop:
  ...
  subgoal by (subst WHILET_unfold) (auto simp: mc_body_def WHILET_exit)
  subgoal by (subst WHILET_unfold) (auto simp: mc_body_def WHILET_exit)
  done
...
lemma add_poly_l_add_poly_p':
  ...
  unfolding add_poly_p'_def add_poly_l_add_poly
  using add_poly_l'_add_poly_p[of \<open>(p, q)\<close> \<open>(p', q')\<close>] assms
